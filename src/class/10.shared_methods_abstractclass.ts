...
abstract class Departmen3 {
  ...
  //abstract methods are that are like signature or architecture paln so that by using this method each child class can implement differnt method
  public abstract printleave(): void; //------------------------------------------------- abstract methods are defenition and they donot have a body and it does not return anything
}
...
class ItDepartment1 extends Departmen3 {
  ...
  public printleave() {
    //------------------------------------------------abstract method
    console.log(`>>>>>>>>>>>>>>>>>>>>abstract method in itdep - 1`);
  }
}
...
class HRDepartment1 extends Departmen3 {
  ...
  public printleave() {
    //------------------------------------------------abstract method
    console.log(`>>>>>>>>>>>>>>>>>>>>abstract method in hrdep -2`);
    if (this.holidays1.length === 0) {
      console.log('there are no holidays');
    }
    console.log(`here is the list of holidays-${Departmen3.name}`); //or we can use this also
    this.holidays1.forEach((holiday: { date: Date; reason: string }, index: number) => {
      console.log(`${index} and ? ${holiday.date} - ${holiday.reason}`);
    });
  }
...
itDepartment1.printleave();
hrDepartment1.printleave();
